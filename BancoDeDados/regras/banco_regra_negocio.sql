...
BEGIN
    ...
    LOOP
        -- verifica se encaixa em alguma das ponts
...
-- FUNCTION: detectar jogo trancado
-- retorna TRUE se nenhum jogador tem jogada possível

CREATE OR REPLACE FUNCTION domino.detectar_jogo_trancado(
    p_idpartida INTEGER,
    p_lado_esquerdo INTEGER,
    p_lado_direito INTEGER
)
...
DECLARE
    v_jogador INTEGER;
BEGIN
    -- verifica todos os jogadores da partida
    FOR v_jogador IN
        SELECT idjogador FROM domino.jogador
        JOIN domino.partida p ON p.idjogo = jogador.idjogo
        WHERE p.idpartida = p_idpartida
    LOOP
        IF domino.verificar_jogadas_possiveis(
            p_idpartida, v_jogador, p_lado_esquerdo, p_lado_direito
        ) = TRUE THEN
            RETURN FALSE;
        END IF;
    END LOOP;
